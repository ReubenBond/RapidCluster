...
internal sealed class MembershipService : IMembershipServiceHandler, ILearnedViewHandler, IAsyncDisposable
{
        ...
        ILogger<MembershipRefreshCoordinator> refreshCoordinatorLogger)
    {
        ...
        _bootstrapController = new BootstrapController(
            _myAddr,
            _clusterOptions,
            _nodeMetadata,
            _log);
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken)
    {
        ...
        if (_seedAddresses.Count == 0)
        {
            ...
            var seedEndpoint = MembershipProtocolMapper.CreateEndpointWithNodeId(_myAddr, nodeId: 1);
    ...
    /// Used for the join path when BootstrapExpect is not set.
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 0; attempt <= options.MaxRetries; attempt++)
        {
            ...
            if (options.CheckBootstrapCoordinator && _bootstrapController.IsBootstrapping && _bootstrapController.ShouldWaitAsCoordinator())
            {
                await _bootstrapController.WaitForCompletionAsync(cancellationToken).ConfigureAwait(true);
    ...
    {
        var newView = MembershipProtocolMapper.BuildMembershipViewFromJoinResponse(response, _options.ObserversPerSubject);
    ...
    private async Task JoinClusterAsync(CancellationToken cancellationToken)
    {
        ...
        var options = new JoinOptions
        {
            ...
            CheckBootstrapCoordinator = _bootstrapController.IsBootstrapping,
    ...
    private async Task<JoinAttemptResult> TryJoinClusterAsync(Endpoint seedAddress, Metadata metadata, CancellationToken cancellationToken)
    {
        ...
        {
            if (_bootstrapController.IsBootstrapping)
            {
        ...
        if (_bootstrapController.IsBootstrapping)
        {
            _bootstrapController.ExitBootstrapMode();
        }
    ...
    {
        var (shouldRedirect, smallestNode) = _bootstrapController.HandleBootstrapInProgressResponse(response, contactedSeed);

        if (shouldRedirect && smallestNode != null)
        {
    ...
    private ConsensusCoordinator? SetMembershipView(MembershipView newView)
    {
        ...
        ReplayBufferedConsensusMessages(_membershipView.ConfigurationId.Version, _stoppingCts.Token);
        ...
        _pendingConsensusMessages.Keys
            .Where(k => k < _membershipView.ConfigurationId.Version)
            .ToList()
    ...
    private RapidClusterResponse HandlePreJoinMessage(PreJoinMessage msg, CancellationToken cancellationToken)
    {
        lock (_membershipUpdateLock)
        {
            ...
            if (_bootstrapController.IsBootstrapping)
            {
                var (response, members) = _bootstrapController.HandlePreJoin(joiningEndpoint, _membershipView.ConfigurationId);

                // If bootstrap was triggered, members will be returned. We need to initialize the view.
                if (members != null)
                {
                    // Build the initial membership view (null configId causes initial ConfigId to be generated)
                    _membershipView = BuildMembershipView(members, maxNodeId: members.Count, configurationId: null);

                    // Update response with the new configuration ID
                    response.JoinResponse.ConfigurationId = _membershipView.ConfigurationId.ToProtobuf();
            ...
            if (statusCode == JoinStatusCode.SafeToJoin)
            {
                ...
                var immutableObservers = _membershipView.GetExpectedObserversOf(joiningEndpoint);
                observers = immutableObservers.ToArray();
                observersCount = observers.Length;
            }

            _log.HandlePreJoinResult(joiningEndpoint, statusCode, observersCount);

            return MembershipProtocolMapper.CreatePreJoinResponse(
                _myAddr,
                _membershipView.ConfigurationId,
                statusCode,
                observers).ToRapidClusterResponse();
        }
    }
    ...
    private async Task<RapidClusterResponse> HandleJoinMessageAsync(JoinMessage joinMessage, CancellationToken cancellationToken)
    {
        ...
        lock (_membershipUpdateLock)
        {
            ...
            if (currentConfiguration == ConfigurationId.FromProtobuf(joinMessage.ConfigurationId))
            {
                ...
                var alertMsg = MembershipProtocolMapper.CreateJoinerAlertMessage(
                    _myAddr,
                    joinMessage.Sender,
                    currentConfiguration,
                    joinMessage.Metadata,
                    joinMessage.RingNumber);
            ...
            else
            {
                ...
                if (_membershipView.IsHostPresent(joinMessage.Sender))
                {
                    ...
                    return MembershipProtocolMapper.CreateSafeToJoinResponse(
                        _myAddr,
                        _membershipView.ConfigurationId,
                        _membershipView.MaxNodeId,
                        configuration.Endpoints,
                        _membershipView.MemberInfos).ToRapidClusterResponse();
                }
                else
                {
                    return MembershipProtocolMapper.CreateConfigChangedResponse(
                        _myAddr,
                        _membershipView.ConfigurationId).ToRapidClusterResponse();
                }
            }
        }
    ...
    private static long GetConfigurationIdFromConsensusMessage(RapidClusterRequest request)
    {
        ...
        {
            RapidClusterRequest.ContentOneofCase.FastRoundPhase2BMessage => request.FastRoundPhase2BMessage.ConfigurationId.Version,
            RapidClusterRequest.ContentOneofCase.Phase1AMessage => request.Phase1AMessage.ConfigurationId.Version,
            RapidClusterRequest.ContentOneofCase.Phase1BMessage => request.Phase1BMessage.ConfigurationId.Version,
            RapidClusterRequest.ContentOneofCase.Phase2AMessage => request.Phase2AMessage.ConfigurationId.Version,
            RapidClusterRequest.ContentOneofCase.Phase2BMessage => request.Phase2BMessage.ConfigurationId.Version,
    ...
    private RapidClusterResponse HandleProbeMessage(ProbeMessage probeMessage, CancellationToken cancellationToken)
    {
        ...
        return MembershipProtocolMapper.CreateProbeResponse(
            _membershipView.ConfigurationId,
            senderInMembership).ToRapidClusterResponse();
    }
    ...
    private RapidClusterResponse HandleMembershipViewRequest(MembershipViewRequest request, CancellationToken cancellationToken)
    {
        ...
        return MembershipProtocolMapper.CreateMembershipViewResponse(
            _myAddr,
            _membershipView).ToRapidClusterResponse();
    }
    ...
    public async ValueTask ApplyLearnedMembershipViewAsync(MembershipViewResponse viewResponse)
    {
        ...
        lock (_membershipUpdateLock)
        {
            ...
            var newView = MembershipProtocolMapper.BuildMembershipViewFromJoinResponse(
                new JoinResponse
                {
                    Endpoints = { viewResponse.Endpoints },
                    MetadataKeys = { viewResponse.MetadataKeys },
                    MetadataValues = { viewResponse.MetadataValues },
                    MaxNodeId = viewResponse.MaxNodeId,
                    ConfigurationId = viewResponse.ConfigurationId
                },
            ...
            if (!isJoined)
            {
                ...
                if (hadPreviousMembership)
                {
                    ...
                    _log.NodeKicked(
                        _myAddr,
                        viewResponse.ConfigurationId.Version,
    ...
    private void EdgeFailureNotification(Endpoint subject, ConfigurationId configurationId)
    {
        ...
        try
        {
            ...
            {
                _log.IgnoringOldConfigNotification(subject, _membershipView, configurationId.Version);
                return;
            }

            _log.AnnouncingEdgeFail(subject, _myAddr, configurationId.Version, _membershipView);

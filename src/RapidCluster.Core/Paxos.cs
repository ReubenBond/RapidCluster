...
internal sealed class Paxos
{
        ...
        ILogger<Paxos> logger)
    {
        ...
        _log.PaxosInitialized(myAddr, configurationId.Version, membershipSize);
    }
    ...
    {
        var messageConfigId = ConfigurationId.FromProtobuf(phase1aMessage.ConfigurationId);
        _log.HandlePhase1aReceived(phase1aMessage.Sender, phase1aMessage.Rank, messageConfigId.Version);

        if (messageConfigId != _configurationId)
        {
            _log.Phase1aConfigMismatch(_configurationId.Version, messageConfigId.Version);
            return;
        }
    ...
    {
        var messageConfigId = ConfigurationId.FromProtobuf(phase1bMessage.ConfigurationId);
        _log.HandlePhase1bReceived(phase1bMessage.Sender, phase1bMessage.Rnd, phase1bMessage.Vrnd, messageConfigId.Version);

        if (messageConfigId != _configurationId)
        {
            _log.Phase1bConfigMismatch(_configurationId.Version, messageConfigId.Version);
            return;
        }
    ...
    {
        var messageConfigId = ConfigurationId.FromProtobuf(phase2aMessage.ConfigurationId);
        _log.HandlePhase2aReceived(phase2aMessage.Sender, phase2aMessage.Rnd, phase2aMessage.Proposal, messageConfigId.Version);

        if (messageConfigId != _configurationId)
        {
            _log.Phase2aConfigMismatch(_configurationId.Version, messageConfigId.Version);
            return;
        }
    ...
    {
        var messageConfigId = ConfigurationId.FromProtobuf(phase2bMessage.ConfigurationId);
        _log.HandlePhase2bReceived(phase2bMessage.Sender, phase2bMessage.Rnd, phase2bMessage.Proposal, messageConfigId.Version);

        if (messageConfigId != _configurationId)
        {
            _log.Phase2bConfigMismatch(_configurationId.Version, messageConfigId.Version);
            return;
        }

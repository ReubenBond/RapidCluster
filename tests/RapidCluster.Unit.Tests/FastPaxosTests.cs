...
public class FastPaxosTests
{
    ...
    public async Task DeclaresSuccess_BeforeAllVotesReceived()
    {
        ...
        var fastPaxos = new FastPaxos(myAddr, new ConfigurationId(1), membershipSize: 5, broadcaster, CreateMetrics(), NullLogger<FastPaxos>.Instance);
    ...
    public async Task DeclaresSuccess_AtExactThreshold(int membershipSize, int expectedThreshold)
    {
        var myAddr = Utils.HostFromParts("127.0.0.1", 1000);
        var broadcaster = new TestBroadcaster();
        var fastPaxos = new FastPaxos(myAddr, new ConfigurationId(1), membershipSize, broadcaster, CreateMetrics(), NullLogger<FastPaxos>.Instance);
    ...
    public void IgnoresVotes_WithWrongConfigurationId()
    {
        var myAddr = Utils.HostFromParts("127.0.0.1", 1000);
        var broadcaster = new TestBroadcaster();
        var fastPaxos = new FastPaxos(myAddr, new ConfigurationId(1), membershipSize: 3, broadcaster, CreateMetrics(), NullLogger<FastPaxos>.Instance);
    ...
    public async Task AcceptsVotes_WithCorrectConfigurationId()
    {
        var myAddr = Utils.HostFromParts("127.0.0.1", 1000);
        var broadcaster = new TestBroadcaster();
        var fastPaxos = new FastPaxos(myAddr, new ConfigurationId(42), membershipSize: 3, broadcaster, CreateMetrics(), NullLogger<FastPaxos>.Instance);
        ...
        for (var i = 0; i < 3; i++)
        {
            var msg = new FastRoundPhase2bMessage
            {
                ConfigurationId = new ConfigurationId(42).ToProtobuf(), // Correct config ID
                Sender = Utils.HostFromParts("127.0.0.1", 1000 + i),
                Proposal = proposal
            };
            fastPaxos.HandleFastRoundProposal(msg);
    ...
    public async Task DetectsVoteSplit_WithThreeProposals()
    {
        ...
        var myAddr = Utils.HostFromParts("127.0.0.1", 1000);
        var broadcaster = new TestBroadcaster();
        var fastPaxos = new FastPaxos(myAddr, new ConfigurationId(1), membershipSize: 9, broadcaster, CreateMetrics(), NullLogger<FastPaxos>.Instance);
    ...
    public async Task SingleNodeCluster_DecidesImmediately()
    {
        var myAddr = Utils.HostFromParts("127.0.0.1", 1000);
        var broadcaster = new TestBroadcaster();
        var fastPaxos = new FastPaxos(myAddr, new ConfigurationId(1), membershipSize: 1, broadcaster, CreateMetrics(), NullLogger<FastPaxos>.Instance);
